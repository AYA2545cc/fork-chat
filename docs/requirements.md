...
| :--- | :--- | :--- |
| `id` | INTEGER | 主キー (自動連番) |
| `conversation_id`| INTEGER | `conversations.id`への外部キー |
| `parent_message_id`| INTEGER | 分岐元メッセージのID (`messages.id`)。ツリー構造の核。 |
| `role` | TEXT | 発言者 ("user" または "model") |
| `content` | TEXT | メッセージ本文 |
| `node_summary`| TEXT | 俯瞰ビュー用要約テキスト |
| `created_at`| TIMESTAMP | 作成日時 |

### 5.3. `attached_files` テーブル
| カラム名 | データ型 | 説明 |
| :--- | :--- | :--- |
| `id` | INTEGER | 主キー (自動連番) |
| `message_id` | INTEGER | `messages.id`への外部キー |
| `file_name` | TEXT | 元のファイル名 |
| `gemini_file_uri` | TEXT | Google File APIの参照URI |
| `created_at`| TIMESTAMP | 作成日時 |
...
| 特定会話の取得 | `GET` | `/api/conversations/{conversation_id}` |
| 会話の削除 | `DELETE`| `/api/conversations/{conversation_id}` |
| 会話タイトルの更新| `PUT` | `/api/conversations/{conversation_id}` |

### 6.2. WebSocket API
| 目的 | エンドポイント |
| :--- | :--- |
| チャット通信 | `ws://your-domain/ws/chat/{conversation_id}` |

* **クライアント→サーバー**: メッセージ送信 (`{ "type": "chat_message", ... }`)
* **サーバー→クライアント**: レスポンスの断片 (`{ "type": "stream_chunk", ... }`), 処理完了通知 (`{ "type": "stream_end", ... }`)
